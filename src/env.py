...
#     Example:
#       { "tasks_file": "...", "task_index": 0, "mcp_config_path": "...json" }
#     """
#     return json.loads(env_config_str)


# def load_task_query(tasks_file: str, task_index: int) -> str:
...
# async def prepare_env_for_task(
#     env_config: Dict[str, Any]
# ) -> Tuple[str, List[Dict[str, Any]], Dict[str, Any], Any]:
#     """
#     - Load task query.
#     - Load MCP tools and sessions.
#     - Return:
#         task_query,
#         tools_schema (for white agent),
#         sessions (for potential evaluation),
#         stack (AsyncExitStack) so executor can close later.
#     """
#     tasks_file = env_config["tasks_file"]
#     task_index = env_config.get("task_index", 0)
#     mcp_config_path = env_config.get("mcp_config_path","src/green_agent/mcp_configs_http.json")
#     # print("#################")
#     # print("mcp_config_path:",mcp_config_path)

#     task_query = load_task_query(tasks_file, task_index)

#     tools, sessions, stack, tools_schema = await load_mcp_tools_with_sessions(
#         mcp_config_path
#     )

#     # For now, executor won’t use `tools` and `sessions` yet, but we return them
#     # so you can later execute/check results from the green side.
#     return task_query, tools_schema, {"sessions": sessions, "tools": tools}, stack
...
# At each step, you must respond with exactly ONE JSON object wrapped in <action>...</action>.

# If you want to call a tool, use:
...
# If you want to stop and give your final answer, use:
